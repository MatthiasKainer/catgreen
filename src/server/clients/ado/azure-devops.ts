...
import {
  AzureDevOpsDefinitions,
  AzureDevOpsBuilds,
  AzureDevOpsBuildResults,
} from "./type";
...
export const client = (name: string) => {
  ...
  const builds = (definition: string) =>
    f(`build/builds?definitions=${definition}&api-version=6.0`);
  ...
  const load = async () => {
    ...
    const buildResults = (
      await Promise.all(
        definitionResults.value.map(({ id }) =>
          limit(() => builds(id)<AzureDevOpsBuildResults>()),
        ),
      )
    )
      .reduce(
        (prev, curr) => [...prev, curr.value?.[0]],
        [] as AzureDevOpsBuilds[],
      )
    ...
    return buildResults.map<Build>((build) => ({
      ...
      meta: {
        ...
        blame:
          build.triggerInfo["pr.sender.name"] ?? build.requestedFor.displayName,
