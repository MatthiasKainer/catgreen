...
export const client = (org: string) => {
  ...
  const load = async () => {
    ...
    const updateTimestamp = (build: Build, createdAt: string) =>
      build.meta.timestamp > createdAt
        ? { timestamp: build.meta.timestamp, when: build.meta.when }
        : { timestamp: createdAt, when: getTimeAgo(createdAt) };

    const buildResult = (build: Build, commit: GithubCheckSuite): Build => ({
      ...build,
      result: compareResults(
        build.result,
        mapResult(commit.conclusion ?? "unknown", commit.status),
      ),
    ...
    const toBuild = (repository: GithubRepository): Build | null => {
      ...
      const checks = lastCommit.checkSuites.nodes.filter(
        (check) => !!check.workflowRun,
      );
